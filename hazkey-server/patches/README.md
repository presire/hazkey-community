...
- **0006 — retired.** Formerly `0006-zenzai-preserve-user-dictionary.patch` (when `review()` received the same `fixRequired` or `wholeResult` constraint twice, preserved a candidate containing `.isFromUserDictionary` by taking the existing give-up path instead of enabling `ignoreMemoryAndUserDictionary`; the existing retry remained for `.isLearned`-only candidates). Retired on 2026-09-08 when upstream commit `ad714fe` (#357, accepted-prediction prefix constraint) changed the `normalizedZenzConstraint()` call sites in `zenzai.swift` and broke the patch's apply context. The fix is now carried as a commit on the converter fork: `presire/AzooKeyKanaKanjiConverter`, `hazkey` branch, commit `2cef753 hazkey: preserve explicitly registered user-dictionary candidates across constraint retries` (baked in as part of merging `ad714fe` in `cfebb64`). `build_swift.cmake`'s marker guard and the CI XCTest anchor guard would both have skipped the apply step anyway, but the dead patch file and its apply blocks were removed. The patch file was removed from this directory.
...
0001, 0004, and 0005 all touched files (`ZenzContext.swift`, vendored `llama.cpp` headers) inside the `AzooKeyKanaKanjiConverter` checkout that upstream refactored twice already, which broke the patches' apply-site assumptions and forced repeated maintenance. 0006 joined them on 2026-09-08 for the same reason: upstream `ad714fe` refactored the `normalizedZenzConstraint()` call sites in `zenzai.swift`, breaking its context. All four are now ordinary commits on `presire/AzooKeyKanaKanjiConverter`'s `hazkey` branch, which `hazkey-server/Package.swift` depends on directly, so there is nothing left to apply for them at build time. 0003 remains the only apply-at-build patch because its target file (`JapaneseNumber.swift`) has been stable and the patch still applies cleanly; there was no forcing reason to fold it into a fork commit yet.
...
There is now one converter patch left to apply at build time: 0003. `hazkey-server/build_swift.cmake` runs a `swift package resolve` step first (to materialize the SwiftPM scratch checkout), then applies 0003 unconditionally against that checkout, then runs the actual `swift build`. The former `0001 -> 0004 -> 0005 -> 0003` chain no longer exists in `build_swift.cmake` because 0001/0004/0005/0006 have no apply step at all now. `.github/workflows/test.yml` mirrors this: it resolves the package, applies the 0003 anchor-guarded patch, then runs `swift test --traits ZenzaiSupport`.
...
- The CI XCTest job (`.github/workflows/test.yml`) uses an ordinary `swift package resolve` (not a scratch path), so its checkout is `.build/checkouts/AzooKeyKanaKanjiConverter`; it applies the same 0003 patch with its anchor guard before `swift test --traits ZenzaiSupport`.
- No workflow file references 0001, 0004, 0005, or 0006 anymore (`.github/workflows/test.yml` and `.github/workflows/build.yml` were checked; only the 0003 apply block remains).
...
3. 0003 remains the only standalone patch; revalidate it against the new fork tip before trusting `build_swift.cmake`'s idempotency check (it uses the generic `hazkey-community patch` string in `JapaneseNumber.swift`). Retired patches (0001, 0004, 0005, 0006) live as fork commits; an upstream merge that touches their target files requires checking the baked-in behavior still compiles and works, not patch revalidation.
