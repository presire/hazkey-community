...
本リポジトリは [7ka-Hiira/hazkey](https://github.com/7ka-Hiira/hazkey) をベースにしたコミュニティ版で、現在のバージョンは **v0.2.29** です。  
...
| 日本全国の地名辞書 | 都道府県・郡・市区町村・町域/大字のうち、変換エンジンが一発変換できない難読地名47,934件を組み込み辞書として収録 (設定UI「住所辞書」、デフォルトON)<br>一発変換できる地名は収録しないため、通常の変換候補の順位に影響しない |
